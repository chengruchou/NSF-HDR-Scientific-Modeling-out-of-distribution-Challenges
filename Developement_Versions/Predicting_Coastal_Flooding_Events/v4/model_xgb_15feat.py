...
# ⚠️ 這裡使用 v5 的全套特徵 (記得要先跑 train_final_v5.py 生成模型)
FEATURES = [
...
# ⚠️ 關鍵修改：為了刷榜 (F1)，我們把門檻降得很低
# 0.35 -> 0.15 (讓模型更傾向於預測淹水，衝高 Recall 和 F1)
BEST_THRESHOLD = 0.01

class FloodModel:
    def __init__(self, model_path='xgb_models_all_feat.pkl'):
        self.models = None
    ...
    def generate_submission(self, df_hourly, test_index_path, output_path):
        ...
        for stn, group in test_intervals.groupby('station_name'):
            ...
            # Sigmoid Adjustment
            # 這裡用 0.15 當作基準，把 >0.15 的機率拉大到 >0.5
...
if __name__ == "__main__":
    ...
    model = FloodModel('xgb_models_all_feat.pkl')
    model.load_model()
